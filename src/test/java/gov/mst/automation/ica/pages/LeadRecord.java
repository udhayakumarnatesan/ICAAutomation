...
public class LeadRecord {
	...
	// This method is used to verify the Lead record data
	public static void verifyLead(WebDriver driver, String sheetName, String tcID) throws Exception {
		...
		verifyApplication(leadpage.leademployeesocialsecuritynumber,
				DataUtils.readExcel(sheetName, tcID, "Employee Social Security Number"));
		verifyApplication(leadpage.leademployeebirthdate, DataUtils.readExcel(sheetName, tcID, "Employee Birth Date"));
		verifyApplication(leadpage.leademployeehomeaddress,
				DataUtils.readExcel(sheetName, tcID, "Employee Home Address"));
		...
		verifyApplication(leadpage.leademployerfeinnumber,
				DataUtils.readExcel(sheetName, tcID, "Employer Fein Number"));
		verifyApplication(leadpage.leademployerofficeaddress,
				DataUtils.readExcel(sheetName, tcID, "Employer Office Address"));
		...
		verifyApplication(leadpage.leadaddateemployernotified,
				DataUtils.readExcel(sheetName, tcID, "AD Date Employer Notified of Injury"));
		verifyApplication(leadpage.leadadlastdateofworkafterinjury,
				DataUtils.readExcel(sheetName, tcID, "AD Last Date of Work After Injury"));
		verifyApplication(leadpage.leadaddidinjuryoccur, DataUtils.readExcel(sheetName, tcID, "Did Injury Occur"));
		verifyApplication(leadpage.leadadwhatwastheinjury,
				DataUtils.readExcel(sheetName, tcID, "AD What was the Injury"));
		verifyApplication(leadpage.leadadpartofbodyinjured,
				DataUtils.readExcel(sheetName, tcID, "AD Part of the Body Injured"));
		verifyApplication(leadpage.leadadpartofbodyinjurytable,
		...
		verifyApplication(leadpage.leadadwasemployeetreatedinanemergencyroom,
				DataUtils.readExcel(sheetName, tcID, "AD Was Employee Treated in an Emergency Room"));
		verifyApplication(leadpage.leadadwasemployeehospitalizedovernight,
				DataUtils.readExcel(sheetName, tcID, "AD Was Employee Hospitalized Overnight"));
		verifyApplication(leadpage.leadadisvalidityclaimdoubted,
		...
		verifyApplication(leadpage.leadcoawhatwasemployeedoing,
				DataUtils.readExcel(sheetName, tcID, "COA What was the Employee Doing Just Before the Incidient"));
		verifyApplication(leadpage.leadcoawasworkerinyouremploy,
				DataUtils.readExcel(sheetName, tcID, "COA Was Worker in Your Employ"));
		verifyApplication(leadpage.leadcoadateoflasthire,
				DataUtils.readExcel(sheetName, tcID, "COA Date of Last Hire"));
		verifyApplication(leadpage.leadassubmitteremail,
				DataUtils.readExcel(sheetName, tcID, "AS Submitter Email Address"));
	}

    // This method is used to check the mapping result
	public static void verifyApplication(WebElement ElementID, String value) throws Exception {
		...
		if (portaldata.equals(ExpectedData)) {
			Report.testStepStatus("Value mapped correctly", "Pass", "The value entered in form " + ExpectedData
					+ " and record value is " + portaldata + " both are same verified");
		} else {
			Report.testStepStatus("Value not mapped correctly", "Fail", "The value entered in form " + ExpectedData
